...
const ImageUpload:React.FC<Props>  = ({info,updateInfo,imageUrls,setImageUrls,handleImageChange}) => {
  ...
  return (
    <div>
        <div className='mb-10'>
            <CldUploadWidget uploadPreset='bsdrd1mz' onUpload={onupload}>
                {({open}:any) => {
        ...
        <div className='grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-10'>
            {imageUrls.map((imageUrl, index) =>(
                <div key={index} className='flex flex-col  justify-center'>
                    <Image width={100} height={200}  src={imageUrl} className='w-[250px] h-[300px] object-cover object-top' alt={`uploades Image ${index + 1}`} />
